    ...
    if (aiKey && reps.length + atts.length > 0) {
      try {
        const aiRes = await unifiedFetch({ body: {
            ...
            messages: [
              { role: "system", content: `You are the "Preparation 360°" brain for student "${firstName}" inside PRACTICE WITH AJIT. Look at the DATA (JSON of their full preparation) and return a STRICT JSON object with these keys ONLY:
{
